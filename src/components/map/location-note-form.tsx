...
export function LocationNoteForm({
  ...
  const {
    ...
    defaultValues: {
      ...
      visibility:
        (existingNote?.visibility as "private" | "friends" | "public") ||
        "private",
    },
  });
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (showVisibilityMenu && !target.closest(".visibility-menu-container")) {
  ...
  useEffect(() => {
    if (isOpen && existingNote?.category) {
      console.log("📝 LocationNoteForm: Setting category from existingNote", {
        category: existingNote.category,
      });
                      ...
                      <div className="absolute bottom-full right-0 mb-2 w-64 bg-[var(--color-neutral-900)] border border-[var(--color-neutral-700)] rounded-lg shadow-xl z-50 p-2">
                        <div className="space-y-1">
                          ...
                          >
                            ...
                            <div className="flex-1">
                              <div className="font-medium text-sm">Bạn bè</div>
                              <div className="text-xs text-[var(--color-neutral-500)]">
                                Bạn bè của bạn có thể xem
                              </div>
                            </div>
                          </button>
              ...
              >
                {isUploadingImages ? (
                  ...
                  <>
                    ...
                    {!isSubmitting && existingNote && "Cập nhật ghi chú"}
                    {!isSubmitting && !existingNote && "Lưu ghi chú"}
                  </>
                )}
              </Button>
            </div>
